...
global projectdir "$rootdir}/Uruguay"
global rawdatadir "${projectdir}/data"
global datadir    "${projectdir}/proc_data"
...
foreach P in 1 2 3 {
  foreach y in log_count_pos_total log_tot_sales log_count_trans  average_pos_firm total_unique_pos  {
    ...
    *graph 
    graph twoway  ///
      (rcap upper_`y'_`P'd lower_`y'_`P'd bw if bw<=9, fcolor(blue) lwidth(0.3) lcolor(blue))               ///
      ...
      ylabel(`l_`y'')                                               ///
      xlabel(1 "45, 45" 2 "45,30" 3 "45,15" 4 "30,45" 5 "30,30" 6 "30,15" 7 "15,45" 8 "15,30" ///
      9 "15 15" 10 "Optimal ${bl_`y'_`P'},${br_`y'_`P'}",           ///
      labsize(small) angle(forty_five)) graphregion(fcolor(white))  ///
      legend(off) yline(0, lwidth(medthin) lcolor(red))             ///
      ytitle(Point Estimate, size(medium))                          ///
      xtitle(Bandwidth (Weeks to reform), size(medium)) 
    
    graph export "${outdir}/robust_`y'_`P'_d_w_0815.pdf", replace 
  }
}
